...
def _set_tracing_context(ctx: Optional[dict] = None) -> None:
    """Backward-compatible shim to set tracing context for callers expecting
    `_set_tracing_context`.

    The langchain_core package expects `_set_tracing_context` and
    `get_tracing_context` to exist; older/newer langsmith versions may use
    different names. Provide a thin wrapper that sets internal contextvars.
    """
    if ctx is None:
    ...
    # ctx may contain run tree under 'parent' or 'run_tree'
    parent = ctx.get("parent") if isinstance(ctx, dict) else None
    project_name = ctx.get("project_name") if isinstance(ctx, dict) else None
    tags = ctx.get("tags") if isinstance(ctx, dict) else None
    metadata = ctx.get("metadata") if isinstance(ctx, dict) else None
...
def get_tracing_context() -> dict:
    """Backward-compatible shim to return a dict representing current tracing
    context for callers expecting `get_tracing_context`.
    """
    return {
